...
export const remove_decimal = (number) => {
...
const capitalize = (s) =>
  typeof s !== "string"
    ...
    : s
        ...
        .join(" ");
export const name = (s, data) =>
  names[s] ? names[s] : data?.name && data.id === s ? data.name : s && s.length <= 3 ? s.toUpperCase() : capitalize(s);
export const ellipse = (string, length = 10) =>
  !string ? "" : string.length < length * 2 + 3 ? string : `${string.slice(0, length)}...${string.slice(-length)}`;

export const equals_ignore_case = (a, b) => (!a && !b) || a?.toLowerCase() === b?.toLowerCase();

export const params_to_obj = (s) =>
  s &&
  JSON.parse(`{"${s.replace(/&/g, '","').replace(/=/g, '":"')}"}`, (k, v) => (k === "" ? v : decodeURIComponent(v)));
